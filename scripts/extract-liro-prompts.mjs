...
const SUPABASE_URL = process.env.LIRO_SUPABASE_URL;
const ANON_KEY = process.env.LIRO_SUPABASE_ANON_KEY;

if (!SUPABASE_URL || !ANON_KEY) {
  throw new Error("Set LIRO_SUPABASE_URL and LIRO_SUPABASE_ANON_KEY before running this extractor.");
}

const outDir = path.resolve("liro-prompts");
const headers = {
  apikey: ANON_KEY,
...
function renderPromptMarkdown(record, result, source) {
  ...
  if (result?.prompt_text) {
    ...
    lines.push(
      ...
      "",
      `The liro.prompt \`get-prompt\` endpoint returned \`${status}\` for this prompt. This usually means the design is part of Unlimited Access or otherwise gated by the site.`,
      ""
    );
  }
...
async function main() {
  ...
  const source = "https://liro.prompt/";
